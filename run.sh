...
echo "[INFO] Latest  version of Calibre is \"${LATEST_VERSION}\""
echo "[INFO] Current version of Calibre is \"${LOCAL_VERSION}\""
...
if [ ! -d ${APP_HOME}/app/.git ]; then
    ...
    # clone the repository w/o history with "--depth 1"
    echo "[INFO] ... git clone -b ${APP_BRANCH} --single-branch ${APP_REPO} ${APP_HOME}/app -v"
    git clone -b ${APP_BRANCH} --single-branch --depth 1 --no-tags ${APP_REPO} ${APP_HOME}/app -v
fi
